...
func (a *apiServer) GetToken(ctx context.Context, req *authclient.GetTokenRequest) (resp *authclient.GetTokenResponse, retErr error) {
	a.LogReq(req)
	defer func(start time.Time) { a.LogResp(req, resp, retErr, time.Since(start)) }(time.Now())
	...
	_, err := col.NewSTM(ctx, a.etcdClient, func(stm col.STM) error {
		...
		return tokens.Put(hashToken(token), tokenInfo)
	})
	if err != nil {
	...
	return &authclient.GetTokenResponse{token}, nil
}
